  ...
  return (
    <section className="panel p-5 sm:p-6">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-2">
        ...
        <span className="text-xs font-semibold uppercase tracking-[0.2em] text-ink/45">Top movers</span>
      </div>
...
export function Leaderboard({ lossLeaderboard, progressLeaderboard }: LeaderboardProps) {
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <LeaderboardList title="Total kg lost leaderboard" rows={lossLeaderboard} />
      <LeaderboardList title="% progress toward target" rows={progressLeaderboard} />
    </div>
  );
}
